        ...
        if (len(new_pass) >= 6):
            if (re.search("[A-Z]", new_pass) is not None) and (re.search("[a-z]", new_pass) is not None) and (re.search("[0-9]", new_pass) is not None):
                if (special_char.search(new_pass) is not None):
                    ...
                    text_stat = "The new password is missing a symbol."
                    print("The new password is missing a symbol.")
            else:
                text_stat = "Missing lower-case, upper-case or number."
                print("Missing lower-case, upper-case or number.")
        else:
            text_stat = "The new password does not meet the requirements."
