...
class StorageManager:
    def __init__(self, db_name):
        ...
        self.cur.execute(
            "CREATE TABLE IF NOT EXISTS ModelSettings (profileId integer NOT NULL, modelComplexity integer, minDetectionConf real, minTrackConf real)")
        self.conn.commit()
        # self.conn.close()
    ...
    def verifyPassword(self, old_pass, new_pass, new_pass_ver):
        special_char = re.compile('[@_!#$%^&*()<>?/\|}{~:]')
        resultNum = None
        text_stat = ""
        if (len(new_pass) >= 6):
            if (re.search("[A-Z]", new_pass) is not None) and (re.search("[a-z]", new_pass) is not None) and (re.search("[0-9]", new_pass) is not None):
                if (special_char.search(new_pass) is not None):
                    resultNum = self.changePassword(old_pass, new_pass, new_pass_ver)
                else:
                    print("The new password is missing a symbol.")
            else:
                print("Missing lower-case, upper-case or number.")
        else:
            print("The new password does not meet the requirements.")

        if (resultNum == 0):
            text_stat = "The old password in incorrect."
        elif (resultNum == -1):
            text_stat = "Confirmation password does not match the new password."
        elif (resultNum == 1):
            text_stat = "The password was successfully changed."
    ...
    def modifyModelParams(self, model_complexity=1, min_det_conf=0.5, min_track_conf=0.5):
        ...
        self.cur = self.conn.cursor()
        self.cur.execute(
            "UPDATE ModelSettings set modelComplexity=?, minDetectionConf=?, minTrackConf=? WHERE profileId=?",
            (model_complexity, min_det_conf, min_track_conf, 1))
        self.conn.commit()
        self.conn.close()
        return True
    ...
    def getModelParams(self):
        self.conn = sqlite3.connect(self.db_name)
        self.cur = self.conn.cursor()
        self.cur.execute("Select modelComplexity, minDetectionConf, minTrackConf FROM  ModelSettings WHERE profileId=?",
                         (1,))
